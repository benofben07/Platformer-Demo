...
func _physics_process(delta):
	...
	# Handle jump.
	if jumping_pressed and is_on_floor() and not landing:
	...
	update_sprite_animation(direction, jumping_pressed)
...
func update_sprite_animation(direction: float, jumping_pressed: bool):
	if not is_on_floor():
		if not jumping:
	...
	if not direction and not landing:
